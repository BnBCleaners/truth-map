...
<div id="truth-network" style="height: 680px; width: 100%; border-radius: 8px; border: 1px solid var(--md-default-fg-color--lightest); margin: 1.2rem 0; background: var(--md-default-bg-color);"></div>

<p style="font-size: 0.85rem; color: var(--md-default-fg-color--light); margin-top: -0.5rem;">Drag nodes · Scroll to zoom · Click a node or edge for details. Physics can be toggled off for a fixed layout.</p>
...
<script>
(function() {
  // Nodes (core early-origins + key later documentation nodes)
  const nodes = new vis.DataSet([
    ...
    // Extended documentation nodes present in the edge list
  ...
  function initNetwork() {
    ...
    const container = document.getElementById('truth-network');
    if (!container || container._vis) return;
    container._vis = true;
    ...
    const options = {
      nodes: {
        shape: 'dot',
        size: 18,
        font: { size: 13, face: 'system-ui, sans-serif', color: 'var(--md-default-fg-color)' },
        borderWidth: 2,
        shadow: true
      },
      edges: {
    ...
    // After stabilization, optionally freeze physics for cleaner reading
    network.once('stabilizationIterationsDone', function() {
      // leave physics on so user can still drag and have gentle response
    });
  }
